...
public class Reto_Casa_4 extends JPanel
    ...
    public static void main(String[] args) {
        ...
        JFrame frame = new JFrame("Reto4Casa");
        // Al cerrar el frame, termina la ejecución de este programa
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        // Agregar un JPanel que se llama Points (esta clase)
        frame.add(new Reto_Casa_4());
        // Asignarle tamaño
    ...
    public void rotacion(Point3 mover){
        Matrix3x3 matrizt = new Matrix3x3();
        double cos1 = Math.cos(mover.x);
        double sen1 = -Math.sin(mover.y);
        double sen2 = Math.sin(mover.y);
        double cos2 = Math.cos(mover.x);
        tras = new Point3[puntos.length];
        for (int i = 0; i < puntos.length; i++){
            matrizt.x[0][0] = cos1;
            matrizt.x[0][1] = sen1;
    ...
    public void escalonar(Point3 mover){
        Matrix3x3 matrizt = new Matrix3x3();
        int maxx = 0, minx= 0, maxy=0, miny=0;
        for(int i= 0; i < puntos.length-1; i++){
            maxx= Math.max((int)(puntos[i].x), (int)(puntos[i+1].x));
            minx= Math.min((int)(puntos[i].x), (int)(puntos[i+1].x));
            maxy= Math.max((int)(puntos[i].y), (int)(puntos[i+1].y));
            miny= Math.max((int)(puntos[i].y), (int)(puntos[i+1].y));
        }
        int cx = (maxx-minx)/2;
        int cy = (maxy-miny)/2;
        Point3 p = new Point3(cx,cy,1);
        trasladar(p);
        tras = new Point3[puntos.length];
        double sx= mover.x;
        double sy= mover.y;
        ...
        Point3 ip = new Point3(-cx,-cy,1);
        trasladar(ip);
        OrganizarEd("/home/diana/Descargas/CompGráfica/Semana 4/casita.txt" );
        
    }
    
    public void escalonarm(Point3 mover){
        Matrix3x3 matrizt = new Matrix3x3();
        int maxx = 0, minx= 0, maxy=0, miny=0;
        for(int i= 0; i < puntos.length-1; i++){
            maxx= Math.max((int)(puntos[i].x), (int)(puntos[i+1].x));
